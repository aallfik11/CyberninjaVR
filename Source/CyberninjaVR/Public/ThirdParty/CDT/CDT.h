...
template <
	...
	// typename TGetVertexCoordZ,
	// typename TGetVertexSkinWeights
	>
DuplicatesInfo FindDuplicates( //modified by Adam Nieckarz
    TVertexIter first,
    TVertexIter last,
    TGetVertexCoordX getX,
    TGetVertexCoordY getY
    // TGetVertexCoordZ getZ,
    // TGetVertexSkinWeights getSkinWeights
    );
...
template <
    ...
    typename TGetEdgeVertexEnd,
    typename TMakeEdgeFromStartAndEnd>
DuplicatesInfo RemoveDuplicatesAndRemapEdges( //modified by Adam Nieckarz
...
#endif
    	constexpr size_t BigPrimeX = 10631;
    	constexpr size_t BigPrimeY = 21179;
    	T x = FMath::RoundToFloat(xy.x * 10000) / 10000;
    	T y = FMath::RoundToFloat(xy.y * 10000) / 10000;
        return Hasher()(x * BigPrimeX) ^ Hasher()(y * BigPrimeY);
    }
};
} // namespace std
    ...
    )
{
    ...
    for(std::size_t iIn = 0, iOut = iIn; iIn < verticesSize; ++iIn, ++first)
    {
        ...
        tie(it, isUnique) = uniqueVerts.insert(
            std::make_pair(V2d<T>::make(getX(*first), getY(*first)/*, getZ(*first), getSkinWeights(*first)*/), iOut));
        if(isUnique)
        {
            di.mapping[iIn] = iOut++;
            continue;
    ...
    TMakeEdgeFromStartAndEnd makeEdge)
{
    const DuplicatesInfo di =
        FindDuplicates<T>(vertices.begin(), vertices.end(), getX, getY/*, getZ, getSkinWeights*/);
