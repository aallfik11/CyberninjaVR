    ...
    const V2d<T>& d)
{
	...
	// Modified by Adam nieckarz
	// Since we need to get lerped skin weights and z component, I'm going to try lerping on just one of the edges, using t_ab as alpha
	V2d<T> intersection;
	intersection.x = std::fabs(a.x - b.x) < std::fabs(c.x - d.x) ? lerp(a.x, b.x, t_ab)
															     : lerp(c.x, d.x, t_cd);
	intersection.y = std::fabs(a.y - b.y) < std::fabs(c.y - d.y) ? lerp(a.y, b.y, t_ab)
															     : lerp(c.y, d.y, t_cd);
	intersection.z = std::fabs(a.z - b.z) < std::fabs(c.z - d.z) ? lerp(a.z, b.z, t_ab)
															     : lerp(c.z, d.z, t_cd);

	//now lerp between skin weights
	...
	SkinBoneArray V0BoneIndices(a.SkinWeights.InfluenceBones, MAX_TOTAL_INFLUENCES);
	InfluenceArray V0InfluenceWeights(a.SkinWeights.InfluenceWeights, MAX_TOTAL_INFLUENCES);
	SkinBoneArray V1BoneIndices(b.SkinWeights.InfluenceBones, MAX_TOTAL_INFLUENCES);
	InfluenceArray V1InfluenceWeights(b.SkinWeights.InfluenceWeights, MAX_TOTAL_INFLUENCES);
	TSet<FBoneIndexType, DefaultKeyFuncs<FBoneIndexType>, TFixedSetAllocator<32>> JointIndices;
	//32 Elements because TFixedSetAllocator threw an assert about MAX_TOTAL_INFLUENCES * 2, I guess it likes powers of 2 as arguments 
	TArray<BoneInfluence, TFixedAllocator<MAX_TOTAL_INFLUENCES * 2>> OutInfluences;
	//that allocator is worst case scenario, most likely will be ~MAX_TOTAL_INFLUNECES
	...
	for (FBoneIndexType BoneIndex : JointIndices)
	{
		...
		if (V0InfluenceIndex != INDEX_NONE && V1InfluenceIndex != INDEX_NONE)
		{
			...
			uint16 InterpInfluence = FMath::Clamp(FMath::TruncToInt(FMath::Lerp(V0Influence, V1Influence, t_ab)),
			                                      0,
			                                      UINT16_MAX);
		...
		else
		{
			if (V0InfluenceIndex != INDEX_NONE)
			{
				uint16 InterpInfluence = FMath::Clamp(
					FMath::TruncToInt(FMath::Lerp(float(V0InfluenceWeights[V0InfluenceIndex]), 0.0f, t_ab)),
					0,
					UINT16_MAX);
			...
			{
				uint16 InterpInfluence = FMath::Clamp(
					FMath::TruncToInt(FMath::Lerp(0.0f, float(V1InfluenceWeights[V1InfluenceIndex]), t_ab)),
					0,
					UINT16_MAX);
	...
	// todo: Check if heap sort would perform better here
	...
	{
		intersection.SkinWeights.InfluenceBones[InfluenceIndex]   = OutInfluences[InfluenceIndex].Key;
		intersection.SkinWeights.InfluenceWeights[InfluenceIndex] = OutInfluences[InfluenceIndex].Value;
	}
    ...
    std::vector<ConformToEdgeTask>& remaining)
{
    ...
    addNewVertex(
        V2d<T>::make((start.x + end.x) / T(2), (start.y + end.y) / T(2), (start.z - end.z) / T(2), start.SkinWeights),
        noNeighbor);
...
void Triangulation<T, TNearPointLocator>::addSuperTriangle(const Box2d<T>& box)
{
    ...
    const V2d<T> posV1 = {center.x - shiftX, center.y - r, center.z, center.SkinWeights};
    const V2d<T> posV2 = {center.x + shiftX, center.y - r, center.z, center.SkinWeights};
    const V2d<T> posV3 = {center.x, center.y + R, center.z, center.SkinWeights};
    ...
    const std::vector<V2d<T> >& newVertices)
{
    return insertVertices(
        newVertices.begin(), newVertices.end(), getX_V2d<T>, getY_V2d<T>, getZ_V2d<T>, getSkinWeights_V2d<T>);
}
