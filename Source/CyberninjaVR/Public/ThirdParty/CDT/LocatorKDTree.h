...
public:
    ...
    void initialize(const std::vector<V2d<TCoordType> >& points)
    {
        ...
        {
            min = V2d_t::make(std::min(min.x, it->x), std::min(min.y, it->y), std::min(min.z, it->z), min.SkinWeights);
            max = V2d_t::make(std::max(max.x, it->x), std::max(max.y, it->y), std::max(max.z, it->z), max.SkinWeights);
        }
