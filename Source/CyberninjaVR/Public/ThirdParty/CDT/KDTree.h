...
public:
    ...
    KDTree()
        ...
        , m_min(point_type::make(
              ...
              -std::numeric_limits<coord_type>::max(),
              -std::numeric_limits<coord_type>::max(), //modified by Adam Nieckarz
              FSkinWeightInfo()))
...
private:
    ...
    void initializeRootBox(const std::vector<point_type>& points)
    {
        ...
        for(pd_cit it = data.begin(); it != data.end(); ++it)
        {
            const point_type& p = points[*it];
            m_min = point_type::make(
                std::min(m_min.x, p.x), std::min(m_min.y, p.y), std::min(m_min.z, p.z), m_min.SkinWeights); // modified by Adam Nieckarz
            m_max = point_type::make(
                std::max(m_max.x, p.x), std::max(m_max.y, p.y), std::max(m_max.z, p.z), m_max.SkinWeights);
        }
