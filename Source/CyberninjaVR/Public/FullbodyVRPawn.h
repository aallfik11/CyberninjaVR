// Fill out your copyright notice in the Description page of Project Settings.

#pragma once

#include "CoreMinimal.h"
#include "GameFramework/Pawn.h"
#include "FullbodyVRPawn.generated.h"

UCLASS()
class CYBERNINJAVR_API AFullbodyVRPawn : public APawn
{
	GENERATED_BODY()

public:
	// Sets default values for this pawn's properties
	AFullbodyVRPawn();

	// Called every frame
	virtual void Tick(float DeltaTime) override;

	// Called to bind functionality to input
	virtual void SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent) override;

protected:
	// Called when the game starts or when spawned
	virtual void BeginPlay() override;

	UPROPERTY(BlueprintReadWrite, EditAnywhere)
	TObjectPtr<USceneComponent> VROrigin;
	UPROPERTY(BlueprintReadWrite, EditAnywhere)
	TObjectPtr<class UCameraComponent> VRCamera;
	UPROPERTY(BlueprintReadWrite, EditAnywhere)
	TObjectPtr<class UMotionControllerComponent> MotionControllerLeftGrip;
	UPROPERTY(BlueprintReadWrite, EditAnywhere)
	TObjectPtr<UMotionControllerComponent> MotionControllerRightGrip;
	UPROPERTY(BlueprintReadWrite, EditAnywhere)
	TObjectPtr<UMotionControllerComponent> MotionControllerLeftAim;
	UPROPERTY(BlueprintReadWrite, EditAnywhere)
	TObjectPtr<UMotionControllerComponent> MotionControllerRightAim;
	UPROPERTY(BlueprintReadWrite, EditAnywhere)
	TObjectPtr<class USkeletalMeshComponent> PlayerMesh;
	UPROPERTY(BlueprintReadWrite, EditAnywhere)
	float PlayerHeight;
private:
	float PreviousYaw;
};
